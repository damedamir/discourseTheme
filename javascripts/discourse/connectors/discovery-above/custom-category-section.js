...
export default class CustomBannersComponent extends Component {
    ...
    get isAllAccessCategory(){
        return this?.getCategory?.parentCategory?.id == 15;
    }

    get isCustomBannerPage(){
    ...
    get decorativeBannersAbove(){
        ...
        return banners.map(banner => {
            ((this.isAllAccessMember || this.isOpenCategory) || banner.url.includes('/c/micro-tactics/50') )  ? banner.has_access = true : banner.has_access = false;
            return banner;
        } );
    }
    ...
    get subcategoryBanners(){
        ...
        const bannerData = subcategories.map(subCat => {
            ...
            return {
                use_overlay: subCat.id == 46 ? false : true,
                has_access: subCat.id == 46 ? false : true,
                url: subCat.id == 46 ? "#" : subCat.path,
